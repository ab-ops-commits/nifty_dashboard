# Build dashboard data for Nifty Indices
import argparse
import json
import os
    ...
    for ticker in tickers:
        try:
            if ticker in data and not data[ticker].empty:
                ...
                plt.plot(df.index[-60:], df['Close'].iloc[-60:], color='#2196f3', linewidth=2)
                plt.title(f'{ticker_clean} - Last 60 Days', fontsize=14)
